...
# Common issues related with killing NPCs

## 1. The bot is not shooting, completely ignores all NPCs in the map
...
## 2. The bot is trying to kill NPC, but it's not shooting, just circling around.
...
## 3. The bot constantly cancels and resumes attack on NPC
...
## 4. The bot is not able to reconnect or throws to the login page

 Cause: your SID is KO (expired).
 Solution: Don't login to the game from the browser while the bot is running. If you do it, a new SID will be generated for your account, causing the older one (that is used by the bot) to expire and sooner or later, you will be disconnected from the game.
