...
🛠️ 2. The plugin's background is green, but some features are not working
...
Reason: The enabled feature is a module [M] that first must be chosen in the module selection. Go to General Settings and select it, then go back and check if you can access the configuration. If not, go back to General settings and scroll down to the very bottom. The configuration section for this module should be there.
  
 
## 4. The kekka's kamikaze feature is not working 
 
1. The bot is not shooting aliens in the gate, just running away from them
...
2.  The bot is only shooting aliens and doesn't use kamikaze
...
3. The bot chooses the kamikaze module, but P.E.T. behaves like in guard mode

 Possible causes and solutions:
  1.  The P.E.T's HP is too high to fire up (it must be lower than 10%).
  2.  You are running the bot on DarkFlash API or DarkBoat API. On these APIs the kamikaze fires up every second time and currently there is no fix for this issue. If it bothers you too much, switch to Legacy API - this problem doesn't occur there. FIXED
  
5️⃣ Problems related with new API 
 
## 5. The API you had selected is not able to load

1. Error message: The API you had selected is not able to load. You probably do not have the required DLL in your lib folder.
Possible causes and solutions:
- You have a 32-bit java version installed on your PC - reinstall Java or our Runtimes.
...
Reason and solution: Clear cache and restart bot.